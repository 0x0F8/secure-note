import { randomBytes, pbkdf2, createCipher, createDecipher } from 'crypto-browserify'

export function createPassword(length = 64) {
    return randomBytes(length + 10).toString('hex').slice(0, length - 1)
}

export function createKey(password: string, salt: string, iterations = 300000): Promise<string> {
